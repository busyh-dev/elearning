...
@section('mainContent')
    ...
    @push('js')
    <script>
    document.getElementById('exitLesson').addEventListener('click', function (e) {
        e.preventDefault(); // Blocca l'uscita immediata

        Swal.fire({
            title: 'Sei sicuro?',
            text: "Attenzione: Se esci adesso, il progresso della lezione non sarà salvato! Si prega di cliccare su '\Esci dall\' attività\'",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                window.location.href = "{{ url('/') }}"; // Vai alla home
            }
        });
    });
</script>
                        ...
                        <div class="header__wrapper flex-wrap">
                            <div class="header__left d-flex align-items-center">
                                <div class="">
                                    ...
                                    <a class="logo_img" id="exitLesson" href="{{ url('/') }}">
    <img class="p-2" src="{{ getLogoImage(Settings('logo')) }}" width="150"
         alt="{{ Settings('site_name') }}">
</a>
                                </div>
                                <!-- Pulsante per chiudere l'attività -->
                                <button id="closeLessonBtn" class="btn btn-danger ms-3">
                                    <i class="fas fa-times"></i> Esci dall'attività
                                </button>
                                ...
                                <div class="contact_wrap d-flex align-items-center flex-wrap mx-0">
                                    ...
                                    <button
                                        ...
                                        @if (isset($lessons))
                                            ...
                                            <!-- <button id="markCompleteBtn" class="theme_btn small_btn2 p-2 me-2 mr-lg-4 fs-14 completeAndPlayNext" style="display: none;">
...
<script>
    ...
    // Funzione per recuperare l'API SCORM
    function getScormAPI(win) {
        var api = null;
        while (win && !api) {
            api = win.API_1484_11 || win.API; // Cerca prima SCORM 2004, poi SCORM 1.2
            win = win.parent !== win ? win.parent : null; // Cerca nell'iframe padre
        }
        return api;
    }

    // Trova l'API SCORM corretta
    var scormAPI = getScormAPI(window);
    console.log("🔍 SCORM API ottenuta:", scormAPI);

    if (!scormAPI) {
        console.error("⚠️ API SCORM non trovata! Assicurati che il corso sia caricato in un LMS compatibile.");
        return;
    }

    

    // Verifica che la funzione LMSInitialize sia disponibile
    var checkScormAPIInterval = setInterval(function() {
        if (scormAPI && typeof scormAPI.LMSInitialize === 'function') {
            console.log("✅ API SCORM pronta!");
            clearInterval(checkScormAPIInterval); // Fermiamo il polling quando l'API è pronta
            var initialized = scormAPI.LMSInitialize('');
            if (!initialized) {
                console.error("⚠️ Inizializzazione SCORM fallita.");
                return;
            }
        } else {
            console.log("⏳ In attesa che l'API SCORM sia pronta...");
        }
    }, 500); // Verifica ogni 500 ms
    
console.log("🛠️ Contenuto di scormAPI:", scormAPI);
console.log("🔍 Tipo di scormAPI:", typeof scormAPI);
console.log("SCORM 1.2 Detected:", typeof scormAPI?.LMSGetValue === "function");
console.log("SCORM 2004 Detected:", typeof scormAPI?.GetValue === "function");

// Verifica se è SCORM 1.2 o SCORM 2004
if (typeof scormAPI.LMSGetValue === "function") {
    // SCORM 1.2
    sessionTime = scormAPI.LMSGetValue("cmi.core.session_time");
} else if (typeof scormAPI.GetValue === "function") {
    // SCORM 2004
    sessionTime = scormAPI.GetValue("cmi.session_time");
}

if (!sessionTime || sessionTime === "not_set") {
    console.log("⚠️ session_time non impostato.");
} else {
    console.log("📊 Session Time:", sessionTime);
}

if (typeof scormAPI.LMSGetValue === "function") {
    console.log("✅ LMSGetValue è disponibile.");
} else {
    console.error("⚠️ LMSGetValue non è definita o non è una funzione.");
}
    // Determina la versione SCORM
    var isScorm12 = typeof scormAPI.LMSGetValue === "function";  // Controlla SCORM 1.2
    var isScorm2004 = typeof scormAPI.GetValue === "function";  // Controlla SCORM 2004
    console.log("✅ SCORM rilevato:", isScorm2004 ? "SCORM 2004" : "SCORM 1.2");

    // Funzione per recuperare il valore SCORM (versione 1.2)
    function getScormValue(param) {
    if (!scormAPI || typeof scormAPI.LMSGetValue !== "function") {
        console.error("⚠️ SCORM API non disponibile o LMSGetValue non è una funzione.");
        return "";
    }
    
    try {
        console.log("🔍 Recuperando il valore SCORM per:", param);
        const value = scormAPI.LMSGetValue(param);
        console.log("📌 Valore ottenuto:", value);
        return value || "";
    } catch (error) {
        console.error("❌ Errore in LMSGetValue:", param, error);
        return "";
    }
}
...
function getScormData() {
    // if (!scormAPI || typeof scormAPI.LMSGetValue !== "function") {
    //     console.error("⚠️ SCORM API non disponibile.");
    //     return null;
    // }

    console.log("🛠️ Verifica SCORM API...");

       // ✅ Controlla quale API SCORM è disponibile (SCORM 1.2 o SCORM 2004)
       let scormAPI = window.API || window.API_1484_11;

       if (!scormAPI) {
        console.error("❌ SCORM API non trovata!");
        return null;
    }

    // if (!window.API && !window.API_1484_11) {
    //     console.error("⚠️ SCORM API non trovata nel `window`.");
    //     return null;
    // }

        const suspendData = getScormValue("cmi.suspend_data");
    const totalTime = getScormValue("cmi.core.total_time");
    const sessionTime = getScormValue("cmi.core.session_time");

    // // Determina quale API SCORM usare (SCORM 1.2 o SCORM 2004)
    // let scormAPI = window.API || window.API_1484_11;

    if (!scormAPI || typeof scormAPI.LMSGetValue !== "function") {
        console.error("⚠️ SCORM API non disponibile o metodo LMSGetValue non presente.");
        console.log("🛠️ Contenuto di scormAPI:", scormAPI);
        return null;
    }
    ...
    console.log("API SCORM 1.2:", window.API);
console.log("API SCORM 2004:", window.API_1484_11);
// console.log("SCORM 1.2 - LMSGetValue:", window.API?.LMSGetValue("cmi.core.student_id"));
// console.log("SCORM 2004 - GetValue:", window.API_1484_11?.GetValue("cmi.core.student_id"));
    
    // Lista di parametri SCORM comuni
    const scormParams = [
        "cmi.core.student_id",
    ...
    scormParams.forEach(param => {
        try {
            // let value = scormAPI.LMSGetValue(param);
            let value = LMSGetValue("cmi.core.session_time");
            console.log("Valore di cmi.core.session_time:", value);
            let errorCode = scormAPI.LMSGetLastError();

            if (errorCode !== "0") {
                console.error(`❌ Errore su ${param}: Codice errore ${errorCode}`);
                scormData[param] = "error";
            } else {
                console.log(`🔍 ${param}:`, value ?? "⚠️ Non disponibile");
                scormData[param] = value || "not_set";
            }
        } catch (error) {
            console.error(`❌ Errore in LMSGetValue(${param}):`, error);
            scormData[param] = "error";
    ...
    // Assicura che lesson_location non sia null
    scormData["cmi.core.lesson_location"] = scormData["cmi.core.lesson_location"] !== "not_set" ? scormData["cmi.core.lesson_location"] : "unknown";
    scormData["cmi.suspend_data"] = scormData["cmi.suspend_data"] !== "not_set" ? scormData["cmi.suspend_data"] : "unknown";
    scormData["cmi.core.total_time"] = scormData["cmi.core.total_time"] !== "not_set" ? scormData["cmi.core.total_time"] : "00:00:00";
    scormData["cmi.core.session_time"] = scormData["cmi.core.session_time"] !== "not_set" ? scormData["cmi.core.session_time"] : "00:00:00";
    scormData["cmi.core.score.raw"] = scormData["cmi.core.score.raw"] !== "not_set" ? scormData["cmi.core.score.raw"] : "0";
    // var sessionTime = scormAPI.LMSGetValue("cmi.core.session_time");
    // console.log("📊 Dati SCORM - Session Time:", sessionTime);
    console.log("📊 Dati SCORM recuperati:", scormData);
    return scormData;
}

// Funzione modificata per evitare la ricorsione infinita
function LMSGetValue(varname) {
    // Aggiungi una logica per evitare chiamate infinite
    if (varname === "cmi.core.score.raw") {
        return "some value"; // Sostituisci con il valore corretto o un mock per il punteggio
    }
    
    // Esegui il metodo originale di LMSGetValue
    if (typeof scormAPI.LMSGetValue === "function") {
        return scormAPI.LMSGetValue(varname); // Restituisci il valore originale
    }
    return null; // Restituisci null se la funzione non è disponibile
}


    // // Funzione per salvare il progresso SCORM
    // function saveScormProgress(eventType) {
    //     if (!scormAPI) return;

    //     var scormData = getScormData(); // Recupera i dati SCORM

    //     if (!scormData) {
    //         console.error("⚠️ Impossibile recuperare i dati SCORM.");
    //         return;
    //     }

    //     var totalSeconds = parseTimeToSeconds(scormData.totalTime);
    //     var sessionSeconds = parseTimeToSeconds(scormData.sessionTime);
    //     var timeRemaining = Math.max(0, totalSeconds - sessionSeconds);

    //     // Salva i dati tramite fetch
    ...
    //             component: "Tracking",
    //             event: eventType, 
    //             description: "User updated SCORM progress",
    //             orgin: "LMS",
    ...
    //         if (!response.ok) {
    //             throw new Error("Errore nel salvataggio del progresso: " + response.statusText);
    //         }
    //         return response.json();
    //     })
    //     .then(data => {
    //         console.log("✅ Progresso salvato:", data);
    //         if (eventType === "Lesson Closed") {
    //             window.location.href = "{{ url('/') }}"; // Torna alla dashboard
    //         }
    //     })
    //     .catch(error => {
    //         console.error("❌ Errore nel salvataggio del progresso:", error);
    //         if (error instanceof SyntaxError) {
    //             console.error("⚠️ Risposta non valida ricevuta. Controlla l'endpoint '/scorm/save-progress'.");
    //         }
    //     });
    // }
...
function saveScormProgress(eventType) {
    ...
    let lessonLocation = localStorage.getItem("scorm_lesson_location");

    // if (!lessonLocation) {
    //     console.warn("⚠️ Nessun progresso da salvare!");
    //     return;
    // }
    console.log("🟢 Salvataggio progresso SCORM iniziato...");
    ...
    var scormData = getScormData(); // Recupera i dati SCORM
    let datiScorm = getScormData();
    console.log("📢 Dati SCORM finali:", datiScorm);
    if (!scormData) {
        console.error("⚠️ Impossibile recuperare i dati SCORM.");
        return;
    }

    console.log("📊 Dati SCORM:", scormData);

    // var lessonLocation = API_1484_11.GetValue('cmi.location');

    // var totalSeconds = parseTimeToSeconds(scormData["cmi.core.total_time"]);
    // var sessionSeconds = parseTimeToSeconds(scormData["cmi.core.session_time"]);
    ...
    var postData = {
        ...
        description: "User updated SCORM progress",
        orgin: "LMS",
        cmi_core_score_raw: scormData["cmi.core.score.raw"],
        ip_url: window.location.href,
    ...
    console.log("📤 Inviando dati SCORM al server:", postData);
    ...
    .then(response => {
        ...
        if (!response.ok) {
            return response.text().then(text => { throw new Error("Errore: " + text); });
        }
        return response.json();
    })
    .then(data => {
        console.log("✅ Progresso SCORM salvato con successo:", data);
        if (eventType === "Lesson Closed") {
            window.location.href = "{{ url('/') }}"; // Torna alla dashboard
        }
        // ✅ Se autoNext è attivo, vai alla prossima lezione
        // if ($("#autoNext").is(':checked')) {
        //         goToNextLesson();
        //     }
    })
    .catch(error => {
        console.error("❌ Errore nel salvataggio SCORM:", error);
        if (error instanceof SyntaxError) {
            console.error("⚠️ Risposta non valida ricevuta. Controlla l'endpoint '/scorm/save-progress'.");
        }
    });
...
function goToNextLesson() {
        console.log("➡️ Passaggio automatico alla prossima lezione...");
     ...
     // Funzione per convertire il tempo in secondi (se necessario)
    ...
    // Gestire eventi come il completamento della lezione
    videoElement.on('ended', function () {
        $('#markCompleteBtn').show(); // Mostra il bottone al termine del video
    });

    // Pulsante per marcare la lezione come completata
    $('#markCompleteBtn').on('click', function () {
        saveScormProgress("Lesson Completed");
    });

    // Pulsante per chiudere la lezione e salvare il progresso
    $("#closeLessonBtn").on("click", function () {
        saveScormProgress("Lesson Closed");
    });
});
        ...
        @else
            ...
            @push('js')
                <script>
                    ...
                    // ✅ Gestione dello stato di autoNext nel localStorage 
    $("#autoNext").change(function () {
        localStorage.setItem('autoNext', $(this).is(':checked') ? 1 : 0);
    });

    // ✅ Imposta la checkbox in base al valore salvato nel localStorage
    if (localStorage.getItem('autoNext') == 0) {
        $("#autoNext").prop('checked', false);
    }

    $("#autoNext").trigger('change'); // Trigger per aggiornare lo stato
            ...
            <!-- @if ($lesson->host == 'SCORM' || $lesson->host == 'SCORM-AwsS3')
                @if (!empty($lesson->video_url))
                    ...
                    <!-- <iframe class="video_iframe" id="video-id" src="{{ $lesson->video_url }}"  -->
    ...
    @if (!empty($lesson->video_url))
        <iframe class="video_iframe" id="video-id" 
                ...
                @if ($lesson->scorm_version == 'scorm_12') 
                    onbeforeunload="if (window.API) { API.LMSCommit(''); API.LMSFinish(''); }" 
                    onunload="if (window.API) { API.LMSCommit(''); API.LMSFinish(''); }"
                @elseif ($lesson->scorm_version == 'scorm_2004')
                    onbeforeunload="if (window.API_1484_11) { API_1484_11.Commit(''); API_1484_11.Terminate(''); }" 
                    onunload="if (window.API_1484_11) { API_1484_11.Commit(''); API_1484_11.Terminate(''); }"
                @endif>
        </iframe>
    @endif
@endif
