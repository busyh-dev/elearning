...
$('#generate_content').on('click', function (e) {
    ...
    $('#show_ai_icon').removeClass('fa fa-robot');
    $('#show_ai_icon').addClass('fa fa-spinner fa-spin');
    ...
    $.ajax({
                ...
                data.results.map((item, index) => {
                    let html = `<div class="row">
                                <div class="offset-lg-10 col-lg-2">
                                    <button class="float-end primary-btn radius_30px mb-10 fix-gr-bg extraBtn copy_generated_content"
                                        data-clipboard-target="#generated_content" data-btn_id="${index}" onclick="copyResult(${index})" id="copy_generated_content_${index}">
                                        <i class="fa fa-copy  copy_icon" id="copy_icon_${index}"></i>
                            ...
                            <textarea class="lms_summernote" name="" id="generated_content_${index}" cols="30" rows="10"></textarea>`;
        ...
        complete: function (data) {
            $('#show_ai_icon').removeClass('fa fa-spinner fa-spin');
            $('#show_ai_icon').addClass('fa fa-robot');
        }
...
$('#ai_advance_section_collapse').on('click', function () {
    ...
    if ($('#ai_advance_icon').hasClass('fa fa-angle-down')) {
        $('#ai_advance_icon').removeClass('fa fa-angle-down');
        $('#ai_advance_icon').addClass('fa fa-angle-up');
    } else {
        $('#ai_advance_icon').removeClass('fa fa-angle-up');
        $('#ai_advance_icon').addClass('fa fa-angle-down');
    }
})
